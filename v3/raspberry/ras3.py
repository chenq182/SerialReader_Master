...
class ThreadedRequestHandler(SocketServer.BaseRequestHandler):
    ...
    def handle(self):
        ...
        ID = pID*1000+ipid
        try:
            conn = MySQLdb.Connect(host='10.170.31.119',user='server',passwd='123456',port=3306)
            conn.select_db('wifiserver')
        except Exception, e:
        ...
        try:
            ...
            while True:
                ...
                val.append((ID, now, ip, port, ma, v, hz, w, kwh))
                if len(val) >= 12:
                    sql = "INSERT INTO Msg (czID,Time,IP,Port,mA,V,Hz,W,kWh) VALUES " + \
                        "(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
